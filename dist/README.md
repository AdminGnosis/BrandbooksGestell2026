...
Static site wrapping the six GESTELL brandbook volumes with a unified
...
```
.
├── index.html              # Landing / hub with the 6 volumes
├── vol-1.html              # Vol. I   — ADN Estratégico                       (12 secs ·  38 págs)
├── vol-2.html              # Vol. II  — Arquitectura de Marca                 ( 7 secs ·  25 págs)
├── vol-3.html              # Vol. III — Identidad Visual                      (13 secs · 110 págs)
├── vol-4.html              # Vol. IV  — Identidad Verbal                      (10 secs ·  50 págs)
├── vol-5.html              # Vol. V   — Identidad Dinámica & Sensorial        (12 secs ·  90 págs)
├── vol-6.html              # Vol. VI  — Infraestructura Digital               ( 8 secs · 120 págs)
├── assets/
│   ├── chrome.css          # Nav bar, drawer, mobile scaling
│   └── chrome.js           # Navigation behavior
├── favicon.svg             # Brand isotype
├── build.py                # Re-generation pipeline
├── vercel.json             # Vercel static config (clean URLs, caching)
└── README.md
```

**Totales:** 6 volúmenes · 62 secciones · 433 páginas A4.
...
| `Esc`      | Cerrar índice                    |
| `1`–`6`    | (Sólo en la portada) ir al Vol.  |
...
El script:
- Lee los 6 HTML crudos desde `/mnt/project/` (o la ruta indicada en `VOLUMES`).
- Extrae los títulos de cada `section-title-page` (código, título, subtítulo).
- Inyecta IDs de ancla (`cover`, `sec-0`, `sec-1`, …).
- Añade `<link>` a `assets/chrome.css` y `<script>` a `assets/chrome.js` en el `<head>`.
- Embebe la configuración del volumen en `window.__GBOS__`.
- Es **idempotente**: ejecutarlo dos veces produce el mismo output.
